...
After the release is live, update the Homebrew tap and re-verify both installers. The source repository is `openclaw/mcporter`, but `steipete/homebrew-tap` remains the active tap target until an explicit tap migration is approved. The tap formula should install the npm `.tgz`, not the Bun-compiled macOS tarball, because `generate-cli --compile` needs the installed package tree so Bun can resolve `mcporter`, `commander`, and related dependencies when compiling from an empty directory. Keep the macOS tarball on the GitHub release as a direct binary asset, but point Homebrew at `mcporter-<version>.tgz`.

The release-published event takes this path automatically. A recovery run is intentionally frictioned: dispatch **Update Homebrew Tap** with the exact published `v<version>` tag and type `update-homebrew-tap` in `confirm`. Before it can dispatch the external tap, the workflow fetches that immutable published release and requires the matching `mcporter-<version>.tgz` asset, its exact release URL, and GitHub's SHA-256 asset digest. It does not synthesize a formula update from a tag alone, a draft/prerelease, or an unverified artifact.
